...
impl Connection {
    ...
    pub fn enable_encryption(&mut self, shared_secret: &[u8]) {
        let key: [u8; 16] = shared_secret.try_into().expect("shared secret must be 16 bytes");
    ...
    pub async fn read_packet(&mut self) -> anyhow::Result<(i32, BytesMut)> {
        loop {
            if let Some(result) = self.try_parse_packet()? {
                return Ok(result);
            }
    ...
    fn try_parse_packet(&mut self) -> anyhow::Result<Option<(i32, BytesMut)>> {
        ...
        let varint_bytes = self.read_buf.len() - peek.len();

        if peek.remaining() < length {
            return Ok(None);
        }

        self.read_buf.advance(varint_bytes);
        let mut packet_data = self.read_buf.split_to(length);
